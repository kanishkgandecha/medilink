 ...
 * working unchanged.
 */
const SourceDisclosure = ({ result, className = '' }) => {
  ...
  return (
    <div
      role="status"
      className={`rounded-lg border px-3 py-2 text-xs space-y-1 ${toneCls} ${className}`}
    >
      ...
      <p className="opacity-90">{display.explanation}</p>
      {display.fallbackUsed && (
        <p className="font-medium">
          The configured provider was unavailable or unsuitable for this request; this result was not generated by a language model.
        </p>
      )}
    </div>
  )
