  ...
  return createPortal(
    ...
    >
      ...
      <div
        ref={panelRef}
        tabIndex={-1}
        className={`relative w-full max-w-md max-h-[90vh] flex flex-col rounded-2xl shadow-2xl border animate-scale-in
      ...
      >
        ...
        <div className="flex items-start gap-4 p-6 pb-4 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 id={titleId} className={`font-bold text-base ${darkMode ? 'text-white' : 'text-gray-900'}`}>{title}</h3>
            <p id={descId} className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{message}</p>
          </div>
          <button
            onClick={onClose}
        ...
        {children && (
          <div className="px-6 pb-2 overflow-y-auto scrollbar-thin">{children}</div>
        )}

        {/* Actions */}
        <div className={`flex flex-wrap justify-end gap-3 px-6 py-4 border-t flex-shrink-0 ${darkMode ? 'border-gray-700' : 'border-gray-100'}`}>
          <button
            onClick={onClose}
